...
declare global {
  ...
  var __sample_orderStore: Map<string, Order> | undefined;
}

// インメモリストア
// HMR対策：グローバル変数を使用してデータを保持
const orders = globalThis.__sample_orderStore ?? new Map<string, Order>();
if (!globalThis.__sample_orderStore) {
  globalThis.__sample_orderStore = orders;
}
