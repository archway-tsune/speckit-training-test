...
import type { ProductRepository } from '@/samples/contracts/catalog';
...
declare global {
  ...
  var __sample_productStore: Map<string, Product> | undefined;
}

// インメモリストア
// HMR対策：グローバル変数を使用してデータを保持
function initializeProductStore(): Map<string, Product> {
  if (globalThis.__sample_productStore) {
    return globalThis.__sample_productStore;
  }
  const store = new Map<string, Product>(PRODUCTS.map((p) => [p.id, p]));
  globalThis.__sample_productStore = store;
  return store;
}
