...
import type { CartRepository, ProductFetcher } from '@/samples/contracts/cart';
import { productRepository } from './product';

// グローバル変数の型定義（HMR対策）
declare global {
  // eslint-disable-next-line no-var
  var __sample_cartStore: Map<string, Cart> | undefined;
}

// インメモリストア（userId -> Cart）
// HMR対策：グローバル変数を使用してデータを保持
const carts = globalThis.__sample_cartStore ?? new Map<string, Cart>();
if (!globalThis.__sample_cartStore) {
  globalThis.__sample_cartStore = carts;
}
